...
#		b3 = tf.Variable(tf.random_uniform([1],-3e-3,3e-3))
		W3 = tf.Variable(tf.random_uniform([layer2_size,1],1000,1000))
		b3 = tf.Variable(tf.random_uniform([1],1000,1000))
	...
	def train(self,y_batch,state_batch,action_batch):
		...
		if self.time_step%10==0:
			self.summary(y_batch,state_batch,action_batch)                                       
	...
	def summary(self,y_batch,state_batch,action_batch):    
		action_gradients,cost,summary_str=self.sess.run([self.action_gradients,self.cost,self.cost_summary],feed_dict={
			self.y_input:y_batch,
			self.state_input:state_batch,
			self.action_input:action_batch,
			self.is_training: False
            })
		self.file_writer.add_summary(summary_str,self.time_step)
...
#		return tf.Variable(tf.random_uniform(shape,-1/math.sqrt(f),1/math.sqrt(f)))
		return tf.Variable(tf.random_uniform(shape,minval=1000,maxval=1000,seed=42))
