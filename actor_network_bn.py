...
class ActorNetwork:
	...
	def __init__(self,sess,state_dim,action_dim,n_outputs):
		self.time_step = 0
		self.sess = sess
       ...
       self.target_update,\
       self.target_discrete_action_output,\
	...
	def create_network(self,state_dim,action_dim):
		...
		discrete_action_output = discretize(action_output,action_dim, self.n_outputs)
        
		return state_input,action_output,[W1,b1,W2,b2,W3,b3],discrete_action_output,is_training
	...
	def create_target_network(self,state_dim,action_dim,net):
		...
		target_discrete_action_output = discretize(action_output,action_dim, self.n_outputs)
       
		return state_input,action_output,target_update,target_discrete_action_output,is_training
...
#			self.summary(q_gradient_batch,state_batch)         
...
#	def summary(self,q_gradient_batch,state_batch):    
#		parameters_gradients=self.sess.run(self.parameters_gradients,feed_dict={
#			self.q_gradient_input:q_gradient_batch,
#			self.state_input:state_batch,
#			self.is_training: False
#            })
#		print('q_gradient_batch:', q_gradient_batch)
#		print('parameter_gradients', parameters_gradients)    
	...
	def discrete_actions(self,state_batch):
		return self.sess.run(self.discrete_action_output,feed_dict={
			self.state_input:state_batch,
			self.is_training: True            
			})
	...
	def target_discrete_actions(self,state_batch):
		return self.sess.run(self.target_discrete_action_output,feed_dict={
			self.target_state_input:state_batch,
			self.target_is_training: True            
			})
